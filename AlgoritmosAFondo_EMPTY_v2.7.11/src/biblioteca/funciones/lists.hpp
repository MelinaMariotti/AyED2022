...
T removeFirst(Node<T>*& p) //OK
{
   ...
   return t; //el elem que fue removido
}
...
T pop(Node<T>*& p) //OK
{
   return removeFirst<T>(p); //la func ya return el elm removido
}

// ------------------ QUEUE / COLA ------------------
// ------------------     FIFO     ------------------

// queue LINEAL (xq tiene 2 ptrs)
// p == front/head of queue && p == back/tail/rear of queue
template <typename T>
Node<T>* enqueue(Node<T>*& p,Node<T>*& q,T e)
{
   //creamos el nuevo nodo a enlazar
   Node<T>* nuevo = new Node<T>;
   //cargarle los datos
   nuevo->info = e;
   nuevo->sig = NULL;
   ...
   if ( q != NULL){
      //hacer que el sig de q apunte al nuevo y ultimo nodo
      q->sig = nuevo;
   }
   else{
      //si la cola ESTA vacia, nuevo es el 1er nodo de la cola
      p = nuevo;
   }

   //de cualquier forma q siempre debe quedar apuntando al nuevo nodo
   q = nuevo;
...
// p == front/head of queue && p == back/tail/rear of queue
template <typename T>
T dequeue(Node<T>*& p, Node<T>*& q) //OK
{
   //q seguira apuntando al ult nodo;
   return removeFirst<T>(p); //ret el elem removido
}
